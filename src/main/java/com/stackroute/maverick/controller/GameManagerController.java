...
public class GameManagerController {
	...
	@Timed(value = "quiz.game-manager.findGameById", histogram = true, percentiles = { 0.95 }, extraTags = { "version",
			"1.0" })
	@RequestMapping(value = "/mp/category/{categoryId}/{topicName}", method = RequestMethod.GET)
	public ResponseEntity<?> findGameByTopicName(@PathVariable(value = "categoryId") int categoryId,@PathVariable(value = "topicName") String topicName)  {
...
//		list.add(multiGameList); new ArrayList<>();
	...
	public ResponseEntity<?> deleteGameById(@PathVariable(value = "id") String id){
	...
	public ResponseEntity<Game> findByTopicNameUnderACategory(@RequestBody Game game,
		...
		System.out.println("Kafka Data passing from here---->"+ game1);
